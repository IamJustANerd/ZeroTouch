...
# ── App Whitelist ─────────────────────────────────────────────────
ALLOWED_APPS = {
    "word":      "start /MAX winword",
    "winword":   "start /MAX winword",
    "excel":     "start /MAX excel",
    "powerpoint":"start /MAX powerpnt",
    "notepad":   "start /MAX notepad",
    "explorer":  "start /MAX explorer",
    "photos":    "start /MAX ms-photos:",
    "chrome":    "start /MAX chrome",
    "edge":      "start /MAX msedge",
    "file":      'start /MAX ""',
}
...
def _force_maximize():
    """Waits 1.5 seconds and forces the active window to maximize."""
    time.sleep(1.5)
    try:
        ...
        print(f"[bridge_v3] Failed to force maximize: {e}")
...
def run_action():
    ...
    if app_key in ("file", "photos") and file_path:
        ...
        print(f"[bridge_v3 /run] Opening file maximized: {file_path}")
        try:
            subprocess.Popen(f'start /MAX "" "{file_path}"', shell=True)
            threading.Thread(target=_force_maximize, daemon=True).start()
            return jsonify({"status": "success", "message": f"Opened maximized: {file_path}"})
        except Exception as e:
            return jsonify({"status": "error", "message": f"Failed to open: {e}"})
    ...
    print(f"[bridge_v3 /run] Executing: {cmd}")
...
if __name__ == "__main__":
    print("=" * 50)
    print("  ZeroTouch V3  —  Host Bridge")
    print(f"  Listening on  http://0.0.0.0:5003")
